...
def data_gen(a):
    ...
    while not eq: 
        ...
        #print(b)
        #print(a-b)
        if attainedEQ(a,b,0.00001) and cnt!=1:
            #eq = True 
...
for ii in range(len(bound_t)):
    ...
    if abs_bound:
        for i in range((size+boundary)):
            a[i][0] = b_value
            a[0][i] = b_value
            a[i][size+boundary-1] = b_value
            a[size+boundary-1][i] = b_value
    if per_bound:
        for i in range(h):
            a[i][0] = a[i][2]
            a[i][h-1] = a[i][h-3]
    ...
    ani.save(r'absorb_'+str(bound_t[ii])+'.gif',writer=writergif)
    if abs_bound:
